let resolucion = parseFloat(canvas.width*0.01), left_x = parseFloat(canvas.width * -0.5), right_x = parseFloat(canvas.width * 1.5), top_y = parseFloat(canvas.height * -0.5), bottom_y = parseFloat(canvas.height * 1.5), num_columns = Math.round((right_x - left_x) / resolucion), num_rows = Math.round((bottom_y - top_y) / resolucion), flow_field = [], minAngle, step_length = 2;
...
class Pos {
    ...
    isInCanvas() {
        return (this.x >= -0.05*paper.width && this.x <= 1.05*paper.width) && (this.y >= -0.05*paper.width && this.y <= 1.05*paper.width)
    }
